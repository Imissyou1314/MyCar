...
public class WeiZhangChaXunActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        weizhangIntent.putExtra("appId",1604);// 您的appId
        weizhangIntent.putExtra("appKey", "04d3a6bfb70c732f8b5a547f8b37213c");// 您的appKey
        ...
        btn_cpsz.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                intent.setClass(WeiZhangChaXunActivity.this, ShortNameList.class);
        ...
        query_city.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                Intent intent = new Intent();
                intent.setClass(WeiZhangChaXunActivity.this, ProvinceList.class);
        ...
        btn_query.setOnClickListener(new OnClickListener() {
            public void onClick(View v) {
                ...
                bundle.putString("carInfo", GsonUtils.Instance().toJson(car));
//                bundle.putSerializable("carInfo",car);
                ...
                if (result) {
                    intent.setClass(WeiZhangChaXunActivity.this, WeizhangResult.class);
    ...
    private boolean checkQueryItem(CarInfo car) {
        if (car.getCity_id() == 0) {
            Toast.makeText(WeiZhangChaXunActivity.this, "请选择查询地", 0).show();
            return false;
        }

        if (car.getChepai_no().length() != 7) {
            Toast.makeText(WeiZhangChaXunActivity.this, "您输入的车牌号有误", 0).show();
            return false;
        }
        ...
        if (car.getCity_id() > 0) {
            ...
            if (classno > 0) {
                if (car.getChejia_no().equals("")) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入车架号不为空", 0).show();
                    return false;
                }

                if (car.getChejia_no().length() != classno) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入车架号后" + classno + "位",
                            0).show();
                    return false;
                }
            } else if (classno < 0) {
                if (car.getChejia_no().length() == 0) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入全部车架号", 0).show();
                    return false;
                }
            }
            ...
            if (engineno > 0) {
                if (car.getEngine_no().equals("")) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入发动机号不为空", 0).show();
                    return false;
                }

                if (car.getEngine_no().length() != engineno) {
                    Toast.makeText(WeiZhangChaXunActivity.this,
                            "输入发动机号后" + engineno + "位", 0).show();
                    return false;
                }
            } else if (engineno < 0) {
                if (car.getEngine_no().length() == 0) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入全部发动机号", 0).show();
                    return false;
                }
            }
            ...
            if (registno > 0) {
                if (car.getRegister_no().equals("")) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入证书编号不为空", 0).show();
                    return false;
                }
                ...
                if (car.getRegister_no().length() == 0) {
                    Toast.makeText(WeiZhangChaXunActivity.this, "输入全部证书编号", 0).show();
                    return false;
                }
            }
            return true;
