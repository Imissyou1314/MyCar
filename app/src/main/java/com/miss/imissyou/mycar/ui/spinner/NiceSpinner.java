package com.miss.imissyou.mycar.ui.spinner;
...
/**
 * Created by Imissyou on 2016/5/20.
 */
public class NiceSpinner extends TextView {
    ...
    private final ThreadLocal<Drawable> drawable = new ThreadLocal<>();
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (!isArrowHide) {
            ...
            if (basicDrawable != null) {
                drawable.set(DrawableCompat.wrap(basicDrawable));
                if (resId != -1) {
                    DrawableCompat.setTint(drawable.get(), resId);
                }
            }
            setCompoundDrawablesWithIntrinsicBounds(null, null, drawable.get(), null);
        }
    ...
    private void animateArrow(boolean shouldRotateUp) {
        ...
        ObjectAnimator animator = ObjectAnimator.ofInt(drawable.get(), "level", start, end);
    ...
    public void setTintColor(@ColorRes int resId) {
        if (drawable.get() != null && !isArrowHide) {
            DrawableCompat.setTint(drawable.get(), ContextCompat.getColor(getContext(), resId));
        }
    }
}
